...
const DocumentsTab = ({ application }) => {
  ...
  const handleView = async (doc, e) => {
    ...
    try {
      ...
      toast.error(`${t("documents_tab.view_failed")}: ${err.response?.data?.message || err.message}`);
    } finally {
  ...
  const handleDownload = async (doc, e) => {
    ...
    try {
      ...
      toast.error(`${t("documents_tab.download_failed")}: ${err.response?.data?.message || err.message}`);
    } finally {
  ...
  const handleAdd = async () => {
    ...
    if (uploadMode === "file" && !selectedFile) {
      toast.warn(t("documents_tab.select_file_first"));
      return;
    }
    if (uploadMode === "url" && !urlValue.trim()) {
      toast.warn(t("documents_tab.enter_url"));
      return;
    }
    setUploading(true);
    try {
      ...
      toast.success(t("documents_tab.added_success"));
      await refreshDocs();
    } catch (err) {
      toast.error(`${t("documents_tab.add_failed")}: ${err.response?.data?.message || err.message}`);
    } finally {
      setUploading(false);
    }
  };
    ...
    <div className="dt-container">
      ...
      <div className="dt-toolbar">
        ...
        {uploadMode === "file" ? (
          ...
          >
            <FiUpload size={13} />
            {selectedFile ? selectedFile.name : t("documents_tab.choose_file")}
            <input
              ref={fileInputRef}
              type="file"
          ...
          <input
            ...
            placeholder={t("documents_tab.paste_url")}
            value={urlValue}
        ...
        >
          {uploading ? t("documents_tab.adding") : t("documents_tab.add")}
        </button>
      </div>
      ...
      {docs.length === 0 ? (
        <div className="dt-empty">
          ...
          <p className="dt-empty-title">{t("documents_tab.no_docs_title")}</p>
          <p className="dt-empty-text">{t("documents_tab.no_docs_text")}</p>
        </div>
      ) : (
            ...
            return (
              <div key={docId} className="dt-item">
                ...
                <div className="dt-item-actions">
                  ...
                  >
                    <FiEye size={13} />
                    {loadingDocs[`view-${docId}`] ? t("documents_tab.loading") : t("documents_tab.view")}
                  </button>
                  {!(doc.url && !doc.fileId) && (
                    <button
                    ...
                    >
                      <FiDownload size={13} />
                      {loadingDocs[`download-${docId}`] ? t("documents_tab.loading") : t("documents_tab.download")}
                    </button>
                  )}
                </div>
              </div>
