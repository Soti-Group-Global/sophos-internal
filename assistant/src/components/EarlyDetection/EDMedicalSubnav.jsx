...
/**
 * Left sidebar sub-navigation for the "Medical History" tab (assistant interface).
 *
 * Props:
 *   booking                    – full booking object
 *   activeScheduleTab          – current sub-tab key
 *   setActiveScheduleTab       – setter
 *   activeTestId               – currently selected test id (or null)
 *   setActiveTestId            – setter
 *   managedTests               – { [section]: test[] }
 *   managedSectionTabs         – string[] of sections managed with test selection
 *   specialistAccordionOpen    – boolean
 *   setSpecialistAccordionOpen – setter
 *   activeSpecialistTab        – current specialist index
 *   setActiveSpecialistTab     – setter
 *   openTestSettingsModal      – (section) => void
 *   normalizeId                – id normalizer
 *   normalizeSpecialistTitle   – title normalizer
 *   readLocalizedName          – (value) => string
 *   getManagedTestEntries      – (section, testId) => entry[]
 *   setShowTestNoteEditor      – setter
 *   setTestNoteDraft           – setter
 *   setEditingTestNoteId       – setter
 */
const EDMedicalSubnav = ({
  ...
  const subTabs = [
    [
      ...
      t("earlyDiagnosis.specialistConsultation", "Specialist Consultation"),
    ],
    [
      "laboratoryTests",
      t("earlyDiagnosis.laboratoryTests", "Laboratory analysis"),
    ],
    [
      "instrumentalAnalysis",
      t("earlyDiagnosis.instrumentalAnalysis", "Исследования/манипуляции"),
    ],
    [
      "morphologicalResearch",
      t("earlyDiagnosis.morphologicalResearch", "Morphological research"),
    ],
    [
      ...
      <div className="ed-schedule-tabs ed-schedule-tabs--vertical">
        {subTabs.map(([key, label]) => (
          <React.Fragment key={key}>
              ...
              <span className="ed-tab-label-group">
                ...
                {key === "specialistConsultation" && (
                  <ChevronDown
                    ...
                    style={{
                      transform:
                        activeScheduleTab === key && specialistAccordionOpen
                          ? "rotate(180deg)"
                          : "none",
            ...
            {/* Managed section accordion — only tests with files or notes */}
            {managedSectionTabs.includes(key) &&
              isManagedOpen(key) &&
              (() => {
                const filledTests = (managedTests?.[key] || []).filter(
                  (test) => {
                    const entries = getManagedTestEntries(
                      key,
                      normalizeId(test?._id),
                    );
                    return entries.some(
                      (e) =>
                  ...
                  <div className="ed-subnav-test-list">
                    {filledTests.map((test) => {
                      ...
                      return (
                        ...
                        >
                          <span className="ed-subnav-test-name">
