...
const HistoryTab = forwardRef(({ application, patient, onSaved }, ref) => {
  ...
  const handleSaveCatalogItem = useCallback(async () => {
    if (!newTestNameRU.trim() && !newTestNameEN.trim()) {
      toast.error(t("history_tab.enter_test_name", { defaultValue: "Please enter a name." })); return;
    }
    ...
    try {
      if (editingTestId) {
        ...
        const updated = res?.data || res;
        if (labPopupMode === "studiesManipulations") setStudyTests((prev) => prev.map((t) => t._id === editingTestId ? updated : t));
        else setLabTests((prev) => prev.map((t) => t._id === editingTestId ? updated : t));
      } else {
        const res = labPopupMode === "studiesManipulations"
          ? await createApplicationInstrumentalAnalysis(payload)
          : await createApplicationLaboratoryTest(payload);
  ...
  }, [labPopupMode, newTestNameRU, newTestNameEN, editingTestId, t]);
  ...
  const handleSaveForm = useCallback(async () => {
    ...
    try {
      const result = await updatePatientHistoryForm(application.applicationId, { ...form });
      if (result) {
        setForm((prev) => ({
          ...prev,
                  ...
                  <div className="ht-add-modal ht-add-modal--note">
                    <div className="ht-add-modal-header">
                      <span className="ht-add-modal-title">
                        {editingNoteId ? t("history_tab.edit_note", { defaultValue: "Edit Note" }) : t("history_tab.add_note", { defaultValue: "Add Note" })}
                      </span>
