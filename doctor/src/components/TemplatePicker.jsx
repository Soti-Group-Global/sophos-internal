...
import { FiTrash2, FiPlus, FiCheck, FiX, FiRotateCcw, FiEdit2 } from "react-icons/fi";
 ...
 */
const TemplatePicker = ({ templates = [], currentValue = "", onApply, onSave, onUpdate, onDelete }) => {
      ...
      {open && (
        ...
        >
          ...
          <div className="tp-dropdown-head">
            <span>{t("template_picker.trigger_label")}</span>
            <button
              type="button"
          ...
          {templates.length === 0 ? (
            ...
            <ul className="tp-list">
              {templates.map((tpl) => {
                const isActive =
                  (editingTpl && editingTpl._id === tpl._id) ||
                  tpl.content === currentValue;
                return (
                  <li
                    key={tpl._id}
                    className={`tp-item${isActive ? " tp-item--active" : ""}${editingTpl?._id === tpl._id ? " tp-item--editing" : ""}`}
                  >
                    <button
                      type="button"
