 ...
 *   isSaving                   – boolean
 *
 *   // Delete managed test confirm
 *   showDeleteConfirm          – boolean
 *   cancelManagedTestDelete    – () => void
 *   confirmManagedTestDelete   – () => void
 *
...
const EDModals = ({
  ...
  // Delete managed test confirm
  showDeleteConfirm,
  cancelManagedTestDelete,
  confirmManagedTestDelete,
  ...
  return (
    <>
          ...
          <div className="mp-modal-overlay" onClick={() => setShowUploadModal(false)}>
            <div className="mp-modal" onClick={(e) => e.stopPropagation()}>
              <div className="mp-modal-header">
                ...
                <button className="mp-modal-close" onClick={() => setShowUploadModal(false)}>
                  ×
                </button>
              </div>
                    ...
                    {uploadFiles.length > 0 && (
                      <div style={{ marginTop: "12px" }}>
                        <label className="mp-field-label">
                          {t("earlyDiagnosis.filesToUpload")}
                        </label>
          ...
          <div className="mp-modal-overlay" onClick={cancelDeleteNote}>
            ...
            >
              ...
              <div className="mp-modal-footer">
                ...
                <button
                  className="mp-btn-save mp-btn-danger"
      ...
      {/* ── Delete Managed Test Confirmation ── */}
      {showDeleteConfirm &&
        createPortal(
            ...
            <div className="mp-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="mp-modal-body">
                ...
                <div className="mp-section">
                  ...
                  <label className="mp-field-label">{t("earlyDiagnosis.mainPackage")}</label>
                  <div className="mp-packages">
                    {ED_PACKAGES.map((pkg) => (
                      <label
                        key={pkg.id}
                        className={`mp-package-card${
                          manualSelectedPkg === pkg.id ? " mp-package-card--active" : ""
                        }`}
                      >
                        <input
                          type="radio"
                  ...
                  <div className="mp-total-row">
                    ...
                    <span className="mp-total-value">{manualTotal.toLocaleString()} ₽</span>
                  </div>
                </div>
                ...
                <div className="mp-section">
                  <div className="mp-field">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setManualPayment((p) => ({ ...p, paymentStatus: e.target.value }))
                      }
                      className="mp-select"
                    >
                      <option value="paid">{t("earlyDiagnosis.status_paid")}</option>
                      <option value="pending">{t("earlyDiagnosis.status_pending")}</option>
                      <option value="processing">{t("earlyDiagnosis.status_processing")}</option>
                      <option value="failed">{t("earlyDiagnosis.status_failed")}</option>
                      <option value="refunded">{t("earlyDiagnosis.status_refunded")}</option>
                    </select>
                  </div>
                  ...
                  <div className="mp-field">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setManualPayment((p) => ({ ...p, transactionId: e.target.value }))
                      }
                  ...
                  <div className="mp-field">
                    <label className="mp-field-label">{t("earlyDiagnosis.notesLabel")}</label>
                    <textarea
                      value={manualPayment.notes}
                      onChange={(e) =>
                        setManualPayment((p) => ({ ...p, notes: e.target.value }))
                      }
