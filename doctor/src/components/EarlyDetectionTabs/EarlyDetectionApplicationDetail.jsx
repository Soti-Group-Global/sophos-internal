...
import { formatDateISO, formatTimeHHMM, formatAppointmentDateTime } from "../../utils/dateFormat";

// dummy application objects used when real data is unavailable
    ...
    appointments: [
      {
        ...
        serviceType: "Sample Service A",
        prescription: { text: "Take one pill daily.", verificationStatus: "Verified" },
        conclusion: { text: "All good.", verificationStatus: "Verified" },
      },
      {
        ...
        serviceType: "Sample Service B",
        prescription: { text: "Rest and drink water.", verificationStatus: "Disapproved" },
        conclusion: { text: "Follow up next week.", verificationStatus: "Verified" },
      },
    ],
    otherAppointments: [],
    documents: [],
    ...
    appointments: [
      {
        ...
        prescription: { text: "Apply ointment twice a day.", verificationStatus: "Verified" },
        conclusion: { text: "Healing well.", verificationStatus: "Verified" },
      },
    ],
    otherAppointments: [],
    documents: [],
...
const EarlyDetectionApplicationDetail = () => {
  ...
  const fetchApplication = async () => {
    try {
      ...
      if (data && data.data) {
        ...
        // no data returned, use dummy record
        const dummy = DUMMY_APPLICATIONS[0];
        setApplication(dummy);
        setOtherAppointments(dummy.otherAppointments);
        setEditValues({
          prescription: dummy.appointments[0]?.prescription?.text || "",
          conclusion: dummy.appointments[0]?.conclusion?.text || "",
        });
      }
    } catch (err) {
  ...
  const handleVerificationAction = async (type, targetId, fieldKey) => {
    ...
    if (result.dismiss === Swal.DismissReason.cancel) {
      ...
      toast.info(t("appointment.toast.markedAsDisapproved", { field: fieldKey }));
      ...
      toast.success(t("appointment.toast.markedAsVerified", { field: fieldKey }));
    }
  ...
  const generateAndDownloadPDF = async () => {
    ...
    pdf.save(`Early_Detection_Application_${id}.pdf`);
  ...
  const renderTests = () => {
    ...
    return orders.map((order, index) => {
      const status = order.status || t("appointment.statusOrdered");
      ...
      <div className="details-grid">
        <div className="detail-item">
          ...
          <div>
            ...
            <p className="detail-value">
              {application?.patientName || t("appointment.notSpecified")}
            </p>
          </div>
        </div>
        ...
        <div className="detail-item">
          ...
          <div>
            ...
            <p className="detail-value">
              {formatDateISO(application.appointments[0].date) || t("appointment.notSpecified")}
            </p>
          </div>
        </div>
        ...
        <div className="detail-item">
          ...
          <div>
            ...
            <p className="detail-value">
              {`${formatTimeHHMM(application.appointments[0].startTime)} - ${formatTimeHHMM(
                application.appointments[0].endTime
              )}`}
            </p>
          </div>
        </div>
        ...
        <div className="detail-item">
          ...
          <div>
            ...
            <p className="detail-value">{serviceType || t("appointment.notSpecified")}</p>
          </div>
        </div>
      </div>
                    ...
                    {doc.verificationStatus === "Under Review" ? (
                      ...
                      >
                        {doc.verificationStatus === "Verified" && (
                          <>
                            <FiCheckCircle className="status-icon" /> {t("appointment.statusVerified")}
                          </>
                        )}
                        {doc.verificationStatus === "Disapproved" && (
                          <>
                            <FiXCircle className="status-icon" /> {t("appointment.statusDisapproved")}
                          </>
                        )}
                      </span>
                    )}
                ...
                <div className="badge-container">
                  ...
                  >
                    ...
                    {application.appointments[0].prescription
                      ...
                      <>
                        <FiClock className="status-icon review" /> {t("appointment.statusUnderReview")}
                      </>
                    )}
                    {application.appointments[0].prescription
                    ...
                    <div className="comment-header">
                      <div className="comment-meta">
                        ...
                        <span className="comment-time">
                          ...
                          {comment.edited && ` · ${t("appointment.edited")}`}
                        </span>
                      </div>
          ...
          <div className="card-content">
            {otherAppointments.map((appt, index) => (
              ...
              >
                ...
                >
                  <p>
                    <strong>{t("patient.email")}:</strong> {appt.doctorEmail}
                  </p>
                  <p>
                  ...
                  <p>
                    ...
                    {`${formatTimeHHMM(appt.startTime)} - ${formatTimeHHMM(
                      appt.endTime
                    )}`}
                  </p>
                  {appt.location && (
                    <p>
                      <strong>{t("appointment.location")}:</strong>{" "}
                      {appt.location}
                    </p>
                  )}
                </div>
