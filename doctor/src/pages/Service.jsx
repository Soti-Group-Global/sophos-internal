...
const Service = ({ applicationId }) => {
    ...
    const myPositionIds = useMemo(() => new Set(myPositions.map((p) => p._id)), [myPositions]);
    ...
    const tabPositions = useMemo(() => {
        ...
        return positions;
    }, [activeTab, positions, myPositions, myPositionIds]);

    const filteredPositions = useMemo(() => {
        const query = searchQuery.trim().toLowerCase();
        if (!query) return tabPositions;
        return tabPositions.filter((p) =>
            [p.name, p.serviceCode, p.pmuCode, p.code]
                .filter(Boolean)
