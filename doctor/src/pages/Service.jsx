...
const TABS = [
    { key: "all",   labelKey: "service.tabAll",   defaultLabel: "All" },
    { key: "my",    labelKey: "service.tabMy",    defaultLabel: "My Services" },
    { key: "other", labelKey: "service.tabOther", defaultLabel: "Other Services" },
];
...
const Service = ({ applicationId }) => {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                // backend returns { doctor: {...} } inside res.data
                const doc = res?.data?.doctor || res?.doctor || res?.data || res;
                const ids = (doc?.specialtyIds || [])
                    .map((s) => String(s?._id || s))
                    .filter(Boolean);
                if (ids.length) setMySpecialityIds(new Set(ids));
            } catch {}
        })();
    }, []);

    // Load positions for ALL doctor specialities when drawer opens
    useEffect(() => {
        if (!drawerOpen || mySpecialityIds.size === 0) return;
        (async () => {
            try {
                const results = await Promise.all(
                    [...mySpecialityIds].map((specId) =>
    ...
    const loadFolder = useCallback(async () => {
        setLoading(true);
        try {
            const params = { parent: currentFolder || "root" };
            if (activeTab === "my" && mySpecialityIds.size > 0) {
                params.specialities = [...mySpecialityIds].join(",");
            }
            const data = await getServiceCategoryFolderContents(params);
    ...
    }, [currentFolder, t, activeTab, mySpecialityIds]);
    ...
    const visiblePositions = useMemo(() => {
        if (activeTab === "my")    return positions.filter((p) => myPositionIds.has(p._id));
            ...
            {drawerOpen && createPortal(
                <>
                    ...
                    <div className="svc-drawer">
                        ...
                        {/* Tabs */}
                        <div className="svc-drawer-tabs">
                            {TABS.map((tab) => (
                                <button
                                    key={tab.key}
                                    type="button"
                                    className={`svc-drawer-tab${activeTab === tab.key ? " active" : ""}`}
