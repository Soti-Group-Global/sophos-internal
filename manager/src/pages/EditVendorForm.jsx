...
function EditVendorForm() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [vendorResponse, specialtiesResponse] = await Promise.all([
          getVendorById(id),
          getSpecialties(1, 100),
        ]);
        const vendor = vendorResponse.vendor;
        setFormData({
          name: vendor.name || '',
          vendorId: vendor.vendorId || '',
          orderSeries: vendor.orderSeries || '',
          email: vendor.email || '',
          phone: vendor.phone || '',
          ...
          services: vendor.services.length > 0
            ? vendor.services.map(service => ({
                specialtyId: service.specialtyId._id,
                tests: service.selectedTests.map(test => ({
                  testId: test.testId,
                  name: test.name,
                })),
              }))
        ...
        setImagePreview(vendor.profilePicture ? `data:image/jpeg;base64,${vendor.profilePicture}` : defaultUser);
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        ...
        await updateVendor(id, formDataToSend);
