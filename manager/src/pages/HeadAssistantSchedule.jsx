...
import {
  ...
  getHeadAssistantAvailability,
  postHeadAssistantAvailability,
  deleteHeadAssistantAvailability,
} from "../utils/api";
import "../styles/Schedule.css";

const HeadAssistantSchedule = ({ email: emailProp }) => {
  const { t } = useTranslation("schedule");
  const params = useParams();
  const email = emailProp || params.email;
  const [headAssistant, setHeadAssistant] = useState(null);
  ...
  useEffect(() => {
    if (!email) {
      ...
      setLoading(false);
      navigate("/head-assistants");
      return;
    }

    const fetchHeadAssistantAndAvailability = async () => {
      try {
        const headAssistantResponse = await getHeadAssistantByEmail(email);
        if (!headAssistantResponse.data.headAssistant) {
          throw new Error("Head Assistant not found");
        }
        setHeadAssistant(headAssistantResponse.data.headAssistant);
        await initializeDays(headAssistantResponse.data.headAssistant.email);
      } catch (error) {
        toast.error(t("error_fetch_head_assistant"));
        navigate("/head-assistants");
      }
    };
    fetchHeadAssistantAndAvailability();
  }, [email, navigate, t]);

  const initializeDays = async (headAssistantEmail) => {
...
const availabilityResponse = await getHeadAssistantAvailability(
  headAssistantEmail.toLowerCase(),
  startDate.toISOString(),
  endDate.toISOString()
);

// Handle both raw array or axios-style { data: [...] }
...
if (availabilityData.length === 0) {
  toast.warn("Received invalid availability data; showing default schedule");
}
  ...
  return (
    slotDate.getFullYear() === day.date.getFullYear() &&
    slotDate.getMonth() === day.date.getMonth() &&
    slotDate.getDate() === day.date.getDate()
  );
});
  ...
  const saveAvailability = async () => {
    try {
      if (!headAssistant?.email) {
        throw new Error(t("error_no_head_assistant_email"));
      }
      ...
      try {
        existingAvailability = await getHeadAssistantAvailability(
          headAssistant.email.toLowerCase(),
          startDate.toISOString(),
          endDate.toISOString()
        );
      ...
      days.forEach((day) => {
        if (!day.isAvailable || day.slots.length === 0) {
          ...
          slotsToDelete.push(
            ...daySlots.map((slot) => ({
              id: slot._id,
              email: headAssistant.email.toLowerCase(),
            }))
          );
        } else {
      ...
      if (slotsToDelete.length > 0) {
        await Promise.all(
          slotsToDelete.map(async ({ id, email }) => {
            try {
              await deleteHeadAssistantAvailability(id, email);
            } catch (err) {
      ...
      if (slotsToAdd.length > 0) {
        await Promise.all(
          slotsToAdd.map(async (slot) => {
            try {
              await postHeadAssistantAvailability(headAssistant.email.toLowerCase(), slot);
            } catch (err) {
      ...
      await initializeDays(headAssistant.email.toLowerCase());
        ...
        <div className="modern-header-content">
          ...
          <div className="modern-doctor-info">
            <div className="modern-doctor-avatar">
              {headAssistant?.firstName?.[0]}
              {headAssistant?.lastName?.[0]}
            </div>
            <div className="modern-doctor-details">
              <h1 className="modern-doctor-name">
                {headAssistant?.firstName} {headAssistant?.lastName}
              </h1>
              <p className="modern-doctor-email">{headAssistant?.email}</p>
              <p className="modern-role-badge">{t("head_assistant")}</p>
            </div>
          </div>
        </div>
      </div>
...
export default HeadAssistantSchedule;