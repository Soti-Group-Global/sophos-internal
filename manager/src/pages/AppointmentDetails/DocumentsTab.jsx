...
const DocumentsTab = ({ application }) => {
  const { t } = useTranslation("documents_tab");
  ...
  const handleView = async (doc, e) => {
    e.stopPropagation();
    // URL-based documents: open directly in new tab
    ...
    try {
      ...
      toast.error(`${t("toast_view_error")}: ${err.response?.data?.message || err.message}`);
    } finally {
  ...
  const handleDownload = async (doc, e) => {
    ...
    try {
      ...
      toast.error(`${t("toast_download_error")}: ${err.response?.data?.message || err.message}`);
    } finally {
  ...
  const handleAdd = async () => {
    ...
    if (uploadMode === "file" && !selectedFile) {
      toast.warn(t("toast_select_file"));
      return;
    }
    if (uploadMode === "url" && !urlValue.trim()) {
      toast.warn(t("toast_enter_url"));
      return;
    }
    setUploading(true);
    try {
      ...
      toast.error(`${t("toast_add_error")}: ${err.response?.data?.message || err.message}`);
    } finally {
      setUploading(false);
    }
  };
  ...
  const renderTypeChip = (filename) => {
    ...
    return (
      <span className={`dt-type-chip dt-type-chip--${type}`}>
        {TYPE_LABELS[type]}
      </span>
    );
  };
  ...
  const renderStatus = (status) => {
    ...
    const map = {
      Verified: { icon: <FiCheckCircle size={11} />, cls: "verified", label: t("status_verified") },
      "Under Review": { icon: <FiClock size={11} />, cls: "review", label: t("status_review") },
      Disapproved: { icon: <FiXCircle size={11} />, cls: "disapproved", label: t("status_disapproved") },
    };
    const entry = map[status] || { icon: null, cls: "default", label: status };
    return (
    ...
    <div className="dt-container">
      ...
      <div className="dt-toolbar">
        ...
        <div className="dt-mode-group">
          <button
            className={`dt-mode-btn${uploadMode === "file" ? " dt-mode-btn--active" : ""}`}
          ...
          <button
            className={`dt-mode-btn${uploadMode === "url" ? " dt-mode-btn--active" : ""}`}
        ...
        {uploadMode === "file" ? (
          <label
            className={`dt-file-label${uploading ? " dt-file-label--disabled" : ""}`}
          >
            <FiUpload size={13} />
        ...
        >
          {uploading ? t("btn_adding") : t("btn_add")}
        </button>
      </div>
      ...
      {docs.length === 0 ? (
        <div className="dt-empty">
          ...
          <p className="dt-empty-title">{t("empty_title")}</p>
          <p className="dt-empty-text">{t("empty_text")}</p>
        </div>
      ) : (
          ...
          {docs.map((doc) => {
            ...
            return (
              <div key={docId} className="dt-item">
                <div className="dt-item-icon-col">
                  {renderTypeChip(filename)}
                </div>
                ...
                <div className="dt-item-body">
                  ...
                  <div className="dt-item-meta">
                    {uploadedAt && (
                      <span className="dt-item-date">{uploadedAt}</span>
                    )}
                ...
                <div className="dt-item-actions">
                  <button
                    ...
                    disabled={loadingDocs[`view-${docId}`]}
                  >
                    <FiEye size={13} />
                    {loadingDocs[`view-${docId}`] ? t("btn_loading") : t("btn_view")}
                  </button>
                  {!(doc.url && !doc.fileId) && (
                    <button
                      className="dt-action-btn dt-action-btn--download"
                      onClick={(e) => handleDownload(doc, e)}
                      disabled={loadingDocs[`download-${docId}`]}
                    >
                      <FiDownload size={13} />
                      {loadingDocs[`download-${docId}`] ? t("btn_loading") : t("btn_download")}
                    </button>
                  )}
                </div>
              </div>
