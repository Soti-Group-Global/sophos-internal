...
import { patchPatient, getDoctorsProfileData } from "../../utils/api";
            ...
            <div className="adp-patient-card">
              ...
              <div className="adp-patient-info-col">
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  {patient?.patientId && (
                    <span style={{ fontSize: 13, color: "#64748b", fontWeight: 500, background: "#f1f5f9", borderRadius: 6, padding: "2px 8px" }}>
                      {patient.patientId}
                    </span>
                  )}
                </div>
...
/* Helper: translate a stored dropdown value using its OPTIONS array */
const translateOption = (options, value, t) => {
  const opt = options.find((o) => o.key === value);
  return opt && opt.labelKey ? t(opt.labelKey) : value || "";
};
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`appt-status-badge ${getApptStatusClass(status)}`}
    >
      {translatedStatus || "—"}
    </span>
  );
        ...
        <div className="adp-section-body">
          {editing ? (
            <div className="adp-contacts-form">
              ...
              <div className="adp-contact-form-row">
                <span className="adp-contact-form-label">{t("contacts.telegram_id_placeholder")}</span>
                <input
                  className="adp-text-input adp-contact-input"
            ...
            <div className="adp-contacts-form">
              <ViewRow label={t("contacts.telephone")} value={form.phoneNumber} />
              ...
              <ViewRow label={t("contacts.telegram_id_placeholder")} value={form.telegramId} />
            ...
            <div className="adp-doc-form">
              ...
              <div className="adp-doc-row adp-doc-row--3-1">
                ...
                <div className="adp-labeled-input">
                  ...
                  <CustomCalendar
                    value={form.cmipDate}
                    onChange={(date) => setForm((f) => ({ ...f, cmipDate: toDateOnly(date) }))}
              ...
              <div className="adp-doc-row adp-doc-row--half">
                <div className="adp-labeled-input adp-labeled-input--wide">
                  ...
                  <CustomCalendar
                    ...
                    onChange={(date) => setForm((f) => ({ ...f, documentIssuedDate: toDateOnly(date) }))}
                    dateFormat="yyyy-MM-dd"
                  />
                </div>
                <div className="adp-labeled-input adp-labeled-input--wide">
            ...
            <div>
              {hasData ? (
                <>
                  ...
                  <div className="adp-info-grid">
                    <VField label={t("documents.citizenship")} value={translateOption(CITIZENSHIP_OPTIONS, form.citizenship, t)} />
                    <VField label={t("documents.doc_type")} value={translateOption(DOCUMENT_TYPE_OPTIONS, form.documentType, t)} />
...
/* Shared icon-prefixed input wrapper — defined outside component to prevent re-mount on re-render */
...
const getDoctorDisplayName = (doctor, language) => {
  ...
  if (doctor.displayName) {
    return doctor.displayName[lang] || doctor.displayName.en || doctor.displayName.ru || "";
  }
  const name = [
    doctor.lastName?.[lang] || doctor.lastName?.en || doctor.lastName?.ru,
    doctor.firstName?.[lang] || doctor.firstName?.en || doctor.firstName?.ru,
    doctor.middleName?.[lang] || doctor.middleName?.en || doctor.middleName?.ru,
  ]
    .filter(Boolean)
    .join(" ");
...
const DoctorAutocomplete = ({ value, onChange, onDoctorSelect, doctors, language, placeholder, className }) => {
  ...
  const filtered = React.useMemo(() => {
    ...
    const named = doctors.filter((d) => getDoctorDisplayName(d, language));
  ...
  const openDropdown = () => {
    if (inputRef.current) {
      ...
      setDropPos({ top: rect.bottom + window.scrollY, left: rect.left + window.scrollX, width: Math.max(rect.width, 220) });
    }
    setOpen(true);
  };
...
const SpecialityAutocomplete = ({ value, onChange, options, placeholder, className }) => {
  ...
  const openDropdown = () => {
    if (inputRef.current) {
      const rect = inputRef.current.getBoundingClientRect();
      setDropPos({ top: rect.bottom + window.scrollY, left: rect.left + window.scrollX, width: Math.max(rect.width, 180) });
    }
    setOpen(true);
  };
...
const DiseasesSection = forwardRef(({ patient }, ref) => {
  ...
  const confirmRow = (id) =>
    setRows((prev) => {
      ...
      if (row && !row.startDate && !row.endDate && !row.diagnosis && !row.icdCode && !row.doctor) {
        return prev.filter((r) => r.id !== id);
      }
      return prev.map((r) => (r.id === id ? { ...r, confirmed: true } : r));
    });
            ...
            <div key={row.id} className="adp-diag-row adp-diag-row--diseases">
              {editing ? (
                <>
                  <CustomCalendar
                    value={row.startDate}
                    onChange={(date) => updateRow(row.id, "startDate", toDateOnly(date))}
                    dateFormat="yyyy-MM-dd"
                  />
                  <CustomCalendar
                    value={row.endDate}
...
const FinalDiagnosisSection = forwardRef(({ patient }, ref) => {
  ...
  const handleDoctorSelectForRow = (rowId, doctor) => {
    ...
    const specialties = (doctor.specialtyIds || []).map(
      (s) => s[`name_${lang}`] || s.name_en || ""
    ).filter(Boolean);
  ...
  const confirmRow = (id) =>
    setRows((prev) => {
      const row = prev.find((r) => r.id === id);
      if (row && !row.date && !row.diagnosis && !row.icdCode && !row.doctorName && !row.jobTitle && !row.speciality) {
        return prev.filter((r) => r.id !== id);
      }
      return prev.map((r) => (r.id === id ? { ...r, confirmed: true } : r));
    });
                ...
                <div className="adp-labeled-input">
                  ...
                  >
                    {MARITAL_STATUS_OPTIONS.map((o) => (
                      <option key={o.key} value={o.key}>
                        {o.labelKey ? t(o.labelKey) : t("common.all_default")}
                      </option>
                    ))}
                  </select>
                </div>
            ...
            <div className="adp-info-grid">
              {form.maritalStatus && (
                <div className="adp-field">
                  ...
                  <span className="adp-field-value">{translateOption(MARITAL_STATUS_OPTIONS, form.maritalStatus, t)}</span>
                </div>
              )}
              {form.education && (
                <div className="adp-field">
                  <span className="adp-field-label">{t("personal_data.education")}</span>
                  <span className="adp-field-value">{translateOption(EDUCATION_OPTIONS, form.education, t)}</span>
                </div>
              )}
              {form.employment && (
                <div className="adp-field">
                  <span className="adp-field-label">{t("personal_data.employment")}</span>
                  <span className="adp-field-value">{translateOption(EMPLOYMENT_OPTIONS, form.employment, t)}</span>
                </div>
              )}
              {form.placeOfWork && (
                <div className="adp-field">
...
/* â”€â”€ DisabilitySection: view + edit modes â”€â”€ */
                ...
                <div className="adp-disability-period">
                  <div className="adp-disability-period-row">
                    ...
                    <CustomCalendar
                      ...
                      onChange={(date) => setForm((f) => ({ ...f, periodFrom: toDateOnly(date) }))}
                      dateFormat="yyyy-MM-dd"
                    ...
                    <CustomCalendar
                      value={form.periodTo}
                      onChange={(date) => setForm((f) => ({ ...f, periodTo: toDateOnly(date) }))}
                      dateFormat="yyyy-MM-dd"
                      disabled={form.indefinitely}
                    />
                  </div>
            ...
            <div className="adp-contacts-form">
              {hasData ? (
                <>
                  ...
                  <ViewRow label={t("disability.group")} value={translateOption(DISABILITY_GROUP_OPTIONS, form.group, t)} />
                  <ViewRow label={t("disability.type_label")} value={translateOption(DISABILITY_TYPE_OPTIONS, form.type, t)} />
                  <ViewRow
                    label={t("disability.primary_repeated_view")}
                    value={form.primaryRepeated === "Primary" ? t("disability.primary") : form.primaryRepeated === "Repeated" ? t("disability.repeated") : form.primaryRepeated}
                  />
                </>
              ) : (
                <p className="adp-empty-msg">
...
/* â”€â”€ RadiationDosesSection: view + edit modes â”€â”€ */
const RadiationDosesSection = forwardRef(({ patient }, ref) => {
  ...
  const confirmRow = (id) =>
    setRows((prev) => {
      const row = prev.find((r) => r.id === id);
      if (row && !row.date && !row.researchType && !row.effectiveDose && !row.note) {
        return prev.filter((r) => r.id !== id);
      }
      return prev.map((r) => (r.id === id ? { ...r, confirmed: true } : r));
    });
                  ...
                  <div className="adp-dob-gender-row">
                    <div className="adp-labeled-input">
                      ...
                      <CustomCalendar
                        ...
                        onChange={(date) =>
                          setEditReps((prev) =>
                            prev.map((r, i) => (i === idx ? { ...r, birthday: toDateOnly(date) } : r)),
                          )
                        }
                        dateFormat="yyyy-MM-dd"
                      />
                  ...
                  <div className="adp-dob-gender-row">
                    <div className="adp-labeled-input">
                      ...
                      <CustomCalendar
                        ...
                        onChange={(date) =>
                          setEditReps((prev) =>
                            prev.map((r, i) => (i === idx ? { ...r, whenIssued: toDateOnly(date) } : r)),
                          )
                        }
                        dateFormat="yyyy-MM-dd"
                      />
...
const GeneralInformationTab = forwardRef(({ application, patient, onSave, saving, showFooter = true, onSaveAndComplete }, ref) => {
  ...
  }, [patient, onSaveAndComplete]);

  useImperativeHandle(
    ref,
    () => ({
      saveAll: handleSaveAll,
      saveAndComplete: handleSaveAndComplete,
    }),
    [handleSaveAll, handleSaveAndComplete],
  );
  ...
  return (
    <>
      ...
      {showCompleteConfirm &&
        createPortal(
          <div className="adp-confirm-overlay">
            <div className="adp-confirm-dialog">
              <p className="adp-confirm-message">
                {t("footer.confirm_mark_completed", "Do you want to mark this application as completed?")}
              </p>
              <div className="adp-confirm-actions">
                <button
                  className="adp-confirm-cancel-btn"
                ...
                <button
                  className="adp-confirm-ok-btn"
                  onClick={handleSaveAndComplete}
                  disabled={isSaving}
                >
                  {isSaving ? t("footer.saving") : t("footer.confirm", "Confirm")}
                </button>
              </div>
            </div>
          </div>,
      ...
      {showFooter && (
        <div className="adp-sticky-footer">
          ...
          <button
            ...
            disabled={isSaving}
          >
            {isSaving ? t("footer.saving") : t("footer.save_and_completed")}
          </button>
        </div>
      )}
    </>
