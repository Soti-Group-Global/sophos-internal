...
const AssigneePopover = ({ task, projectId, onClose, onUpdate, t }) => {
  ...
  return (
    <div className="modern-popover assignees-popover">
      <div className="popover-header">
        <h4>{t('taskBoard.assignees.title')}</h4>
      </div>
      <div className="popover-content">
        {loading ? (
          <LoadingComponent message={t('taskBoard.assignees.loading')} />
        ) : members.length ? (
...
function TaskBoard({ projectId, currentUserEmail, projectType, userRole, t }) {
  ...
  const formatDate = (dateString) => {
    ...
    if (diffDays === 0) return t('taskBoard.dueDate.today');
    if (diffDays === 1) return t('taskBoard.dueDate.tomorrow');
    if (diffDays === -1) return t('taskBoard.dueDate.yesterday');
    if (diffDays < 0) return `${Math.abs(diffDays)}${t('taskBoard.dueDate.daysAgo')}`;
  ...
  const canAddTask =
    ...
    (projectType === "company" && userRole === "head_manager");
  ...
  const getTranslatedColumnTitle = (columnId) => {
    const translations = {
      todo: t('taskBoard.columns.todo'),
      inprocess: t('taskBoard.columns.inprocess'),
      completed: t('taskBoard.columns.completed')
    };
  ...
  const getTranslatedPriorityLabel = (priority) => {
    const translations = {
      low: t('taskBoard.priority.low'),
      medium: t('taskBoard.priority.medium'),
      high: t('taskBoard.priority.high'),
      critical: t('taskBoard.priority.critical')
    };
      ...
      {projectId ? (
                  ...
                  <div className="modern-column-header">
                    <div className="column-title-section">
                      ...
                      <div className="column-info">
                        <h3 className="modern-column-title">
                          {getTranslatedColumnTitle(column.id)}
                        </h3>
                  ...
                  {isAddingTask === column.id && (
                    <div className="modern-add-task-card">
                      <div className="add-task-header">
                        ...
                        <input
                          type="text"
                          placeholder={t('taskBoard.addTask.placeholder')}
                          value={newTaskTitle}
                      ...
                      >
                                      ...
                                      <div className="task-actions">
                                        ...
                                        {task.priority && (
                                          <div
                                            ...
                                            title={getTranslatedPriorityLabel(task.priority)}
                                          >
                                            <FiFlag size={12} />
                                          </div>
                                        )}
                                            ...
                                            <div className="modern-assignees-container">
                                              ...
                                              {canAssignMembers && (
                                                <button
                                                  ...
                                                  title={t('taskBoard.assignees.manage')}
                                                >
                                                  <FiPlus size={12} />
                                                </button>
                                              )}
                                            ...
                                            <div className="modern-popover menu-popover">
                                              <div className="popover-content">
                                                <button className="modern-menu-item">
                                                  <FiEdit3 size={16} />
                                                  <span>{t('taskBoard.menu.edit')}</span>
                                                </button>
                                                <button className="modern-menu-item">
                                                  <FiMessageSquare size={16} />
                                                  <span>{t('taskBoard.menu.comment')}</span>
                                                </button>
                                                <button className="modern-menu-item">
                                                  <FiPaperclip size={16} />
                                                  <span>{t('taskBoard.menu.attachment')}</span>
                                                </button>
                                              </div>
                                            </div>
                                          )}
                                    ...
                                    <div className="modern-task-meta">
                                          ...
                                          {userRole == "head_manager" ? (
                                            <span className="duo-date-label">
                                              {task.dueDate
                                                ...
                                                : t('taskBoard.dueDate.setDate')}
                                            </span>
                                          ) : (
                                            <span className="duo-date-label">
                                              {task.dueDate
                                                ? formatDate(task.dueDate)
                                                : t('taskBoard.dueDate.noDate')}
                                            </span>
                                          )}
                                        ...
                                        {activeCalendar === task._id &&
                                          canEditDeadline && (
                                            <div className="modern-popover date-popover">
                                              <div className="popover-header">
                                                <h4>{t('taskBoard.dueDate.title')}</h4>
                                              </div>
                                              <div className="popover-content">
                                                <input
                                                  type="date"
                                      ...
                                      <div className="meta-item-wrapper popover-wrapper">
                                        ...
                                        >
                                          ...
                                          <span>
                                            {task.priority
                                              ? `${getTranslatedPriorityLabel(task.priority)} ${t('taskBoard.priority.priority')}`
                                              : t('taskBoard.priority.setPriority')}
                                          </span>
                                        </button>
                                        ...
                                        {activePriority === task._id &&
                                          canEditPriority && (
                                            <div className="modern-popover priority-popover">
                                              <div className="popover-header">
                                                <h4>{t('taskBoard.priority.title')}</h4>
                                              </div>
                                              <div className="popover-content">
                                                {Object.entries(
                                                  priorityLevels
                                                  ...
                                                  ([
                                                    ...
                                                    >
                                                      ...
                                                      <span style={{ color }}>
                                                        {getTranslatedPriorityLabel(key)}
                                                      </span>
                                                    </button>
                                                  )
                                                )}
                        ...
                        {tasks[column.id]?.length === 0 && !isAddingTask && (
                          <div className="modern-empty-state">
                            ...
                            <h4>{t('taskBoard.emptyState.title')}</h4>
                            <p>{t('taskBoard.emptyState.description')}</p>
                            {canAddTask && (
                              <button
        ...
        <div className="modern-no-project">
          <div className="modern-empty-board">
            ...
            <h2>{t('taskBoard.noProject.title')}</h2>
            <p>{t('taskBoard.noProject.description')}</p>
          </div>
        </div>
      )}
    </div>
